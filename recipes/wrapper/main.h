...
#define _pick_implementation(_container, _type)                               \
    ...
    {                                                                         \
        ...
        static const size_t bkt_capacity = n_fields * 2;                      \
        size_t length = root ? reflectc_length(root) : 1;                     \
        size_t i;                                                             \
                                                                              \
        if (!root && !(root = calloc(length, sizeof *root))) {                \
            return NULL;                                                      \
        }                                                                     \
                                                                              \
        for (i = 0; i < length; ++i) {                                        \
            struct reflectc *iter = root + i;                                 \
            memcpy(iter, &_type##__root, sizeof *iter);                       \
            iter->value = self + i;                                           \
            iter->length = length - i;                                        \
        }                                                                     \
        if (!self) return root;                                               \
                                                                              \
        for (i = 0; i < length; ++i) {                                        \
            struct oa_hash_entry *bkts = malloc(sizeof *bkts * bkt_capacity); \
            struct reflectc *fields = malloc(sizeof(_type##__fields));        \
            struct reflectc *iter = root + i;                                 \
            oa_hash_init(&iter->ht, bkts, bkt_capacity);                      \
            memcpy(fields, _type##__fields, sizeof(_type##__fields));
#define _pick_field__(_name, _type)                                           \
            fields->value = &self->_name;                                     \
            oa_hash_set(&iter->ht, #_name, sizeof(#_name) - 1, fields);       \
            ++fields;
#define _pick_field_container(_name, _type)                                   \
            fields->value = &self->_name;                                     \
            oa_hash_set(&iter->ht, #_name, sizeof(#_name) - 1,                \
                reflectc_from_##_type(reflectc_derefer_max(fields), fields)); \
            ++fields;
#define _pick_field_struct _pick_field_container
#define _pick_field_union _pick_field_container
